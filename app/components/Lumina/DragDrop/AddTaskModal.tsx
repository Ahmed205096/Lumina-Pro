...
interface AddTaskModalProps {
  ...
  onTaskCreated: (task: KanbanTask, dbTask: ApiTask) => void;
  todoCount: number;
}
...
export default function AddTaskModal({
  ...
  const [newPeriority, setNewPeriority] = useState<"low" | "medium" | "high">("medium");
  ...
  return (
    ...
    >
      ...
      >
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(192,193,255,0.14),transparent_45%),radial-gradient(circle_at_bottom_right,rgba(93,230,255,0.10),transparent_50%)]" />
        <button
        ...
        <div className="relative">
          ...
          <p className="mb-5 text-sm text-[#c7c4d7]/70">
            This task will be created in the selected workspace.
          </p>
        </div>
        ...
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            if (!workspaceSlug) {
              setCreateError("Please select a workspace first.");
              return;
            }
            if (!newTitle.trim()) {
            ...
            try {
              const res = await fetch("/api/tasks", {
                ...
                body: JSON.stringify({
                  workspaceSlug,
                  content: serializeTaskContent(newTitle, newDescription),
                  periority: newPeriority,
                  status: "todo",
                  order: todoCount,
                  assignedTo: isOwnerOrAdmin ? newAssigneeIds : [],
                  dueDate: isOwnerOrAdmin && newDueDate ? newDueDate : undefined,
                }),
              });

              const data = await safeJson<ApiTask | string>(res);
              if (!res.ok) {
                setCreateError(typeof data === "string" ? data : "Could not create task.");
                return;
              }

              const created = data as ApiTask;
              onTaskCreated(toKanbanTask(created), created);
              
              // Reset state on successful creation
              setNewTitle("");
              setNewDescription("");
              setNewPeriority("medium");
              setNewDueDate("");
              setNewAssigneeIds([]);
            } catch (err) {
              setCreateError((err as Error)?.message || "Could not create task.");
            } finally {
              setIsCreating(false);
            }
          }}
            ...
            <div className="max-h-40 space-y-2 overflow-y-auto rounded-xl border border-white/10 bg-white/3 p-3">
              {!isOwnerOrAdmin ? (
                <p className="text-xs text-[#c7c4d7]/60">
                  Only workspace owner/admin can assign tasks.
                </p>
              ) : members.length === 0 ? (
                <p className="text-xs text-[#c7c4d7]/60">No members found.</p>
              ) : (
                members.map((member) => {
                  const checked = newAssigneeIds.includes(member.id);
                  return (
                    <label
