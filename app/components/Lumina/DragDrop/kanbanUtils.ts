...
export function apiStatusToColumnId(status: ApiTaskStatus): KanbanColumnData["id"] {
...
const accentPalette = [
  "bg-rose-400", "bg-amber-300", "bg-cyan-300",
  "bg-indigo-300", "bg-emerald-300", "bg-violet-300",
];

function stableAccent(id: string) {
  let hash = 0;
...
export function toKanbanTask(apiTask: ApiTask): KanbanTask {
  ...
  const dueLabel = apiTask.dueDate
    ? new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(apiTask.dueDate))
    : undefined;
  const rawPeriority = apiTask.periority || "medium";
  const assignees = (apiTask.assignedTo ?? [])
    .map((u) => ({ id: (u._id || u.id || "").toString(), name: u.name || u.email || "Unknown", image: u.image }))
    .filter((u) => u.id);
